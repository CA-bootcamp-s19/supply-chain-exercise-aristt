...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale (uint _sku) { require(items[_sku].state == State.ForSale, "require(items[_sku].state == State.ForSale)"); require(items[_sku].seller != address(0), "require(items[_sku].seller != address(0))"); _;}
  modifier sold (uint _sku) { require(items[_sku].state == State.Sold, "require(items[_sku].state == State.Sold"); _;}
  modifier shipped (uint _sku) { require(items[_sku].state == State.Shipped, "require(items[_sku].state == State.Shipped"); _;}
  modifier received (uint _sku) { require(items[_sku].state == State.Received, "require(items[_sku].state == State.Received"); _;}
